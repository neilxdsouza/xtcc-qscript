...
void split_defn_into_decl_plus_constructor (const string & defn, vector <string> & decl_vec, vector<string> & constructor_init_vec)
{
	//string test(" struct axis_q5_0 ax_q5_1(q5_1_data, q5_1_ttl_stmt_arr);");

	int pos_left_paren = defn.find ("(");
	string decl = defn.substr(0, pos_left_paren);
	decl += ";";
	decl_vec.push_back (decl);
	cout << "decl: " << decl << endl;

	int pos_2nd_space = defn.rfind (' ', pos_left_paren);
	string constructor_initializer = defn.substr (pos_2nd_space, (defn.length() - pos_2nd_space - 2)); 
	cout << "constructor_initializer: " << constructor_initializer << endl;
	constructor_init_vec.push_back (constructor_initializer);
...
void print_axis_code (FILE * axes_h, FILE * axes_cpp, FILE * axes_drv_func)
{
	...
	for (CMAPITER it = ax_map.begin(); it != ax_map.end(); ++it, ++n_axes_output_counter) {
		...
		if (can_use_stub_hint == true) {
			...
			// 12-sep-2013 disable this - revist after implementing map-reduce
			// Currenly we are generating bad code - need to modify
			// the code generated by the ax_stmt
			has_single_named_var = false;
	...
	fprintf (axes_cpp, "/* ======================================  */\n");
	fprintf (axes_cpp, "struct AxesGroup {\n");
	vector <string> decl_vec;
	vector <string> constructor_init_vec;
	for (CMAPITER it = ax_map.begin(); it != ax_map.end(); ++it, ++n_axes_output_counter) {
		//struct ax* l_ax = *it;
		//cout << "Processing axis: " << it->first.c_str() << endl;
		//bool found_stub_hint = false;
		...
		if (can_use_stub_hint == true) {
			...
			//fprintf (axes_cpp, "%s "
			//		, obj_defn.str().c_str());
			split_defn_into_decl_plus_constructor (obj_defn.str(), decl_vec, constructor_init_vec);
		} else {
			AbstractCountableAxisStatement* iter = it->second->count_ax_stmt_start;
			map <string, SymbolTableEntry*> name_list ;
			while (iter) {
			...
			// for other axes that have the same
			// stub list generated using stub_hint(s)
			// No need in pass 2: it->second->name_list = name_list;
	...
	fprintf (axes_cpp, "/* =============== DECL ==================  */\n");
	for (int i=0; i < decl_vec.size(); ++i) {
		fprintf (axes_cpp, "%s\n", decl_vec[i].c_str());
	}
	fprintf (axes_cpp, "/* =============== CONS INIT ==================  */\n");
	fprintf (axes_cpp, "AxesGroup(): \n");
	for (int i=0; i < constructor_init_vec.size(); ++i) {
		fprintf (axes_cpp, "\t%s", constructor_init_vec[i].c_str());
		if (i <  constructor_init_vec.size() - 1) {
			fprintf (axes_cpp, ",");
		} 
		fprintf (axes_cpp, "\n");
	}
