...
function create_multiple_questions_view (questions_obj_arr, stubs_obj_arr, err_obj_arr, question_text_obj_arr) {
	...
	*/
	...
	if (questions_obj_arr[0].question_type == 'video_q') {
		...
		new_question_view += create_single_question_title (question_text_obj_arr[0]);
		...
		if (questions_obj_arr.length == 1 && questions_obj_arr[0].question_text_arr.length == 1) {
			new_question_view += "<div>" + questions_obj_arr[0].question_text_arr[0] 
			new_question_view += create_single_question_title (question_text_obj_arr[0]);
			new_question_view += "</div>";
			...
			if(is_question_text_same) {
				new_question_view += "<tr><th><p>" + questions_obj_arr[0].question_text_arr[0] + "</p>" ;
				key =  question_text_obj_arr[0].question_text_arr[0].key[0];
				new_question_view += "<p>" + get_translation (key) + "</p>";
				new_question_view += "</th></tr>";
			}
			for (var i=0; i<questions_obj_arr.length; i++) {
				var curr_question_obj = questions_obj_arr[i];
				if(!is_question_text_same) {
				...
				new_question_view += "<input cols='40' rows='8' type='number' value='" + prevValue + "' id='input_" + curr_question_obj.qno + "' name='input_" + curr_question_obj.qno + "'></input>";
		...
		for (var i = 0; i < questions_obj_arr.length;  ++i) {
			...
			for (var j = 0; j < curr_question_obj.question_text_arr.length; ++j) {
				...
				question_title_div.innerHTML += "<p>" + create_single_question_title (question_text_obj_arr[i]) + "</p>";
			}
...
function get_stubs_display_view (question_obj, stubs_obj_arr) {
	my_log ("Enter: get_stubs_display_view: question_obj.no_mpn" + question_obj.no_mpn + " current_language: " + global_survey_related_info.current_language + ", stubs_obj_arr.length: " + stubs_obj_arr.length);
	...
	if (question_type === "nq") {
		...
		if (res2 === null) {
			...
			my_log ("Found stub name : " + res2.name);
		}
...
function ui_create_question_form (questions_obj_arr, stubs_obj_arr, err_obj_arr, question_text_obj_arr) {
	...
	if (result == "single_question") {
		...
		create_grid_questions_view (questions_obj_arr, stubs_obj_arr, question_text_obj_arr);
	} else {
...
function set_lang (the_lang) {
	my_log ("set_lang the_lang: " + the_lang);
