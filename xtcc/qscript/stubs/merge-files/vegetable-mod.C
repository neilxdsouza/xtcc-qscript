...
DummyArrayQuestion * find_q( vector <AbstractQuestion*> & question_list, string qname, stringstream & qax_stream);
...
qtm_data_file_ns::QtmDataDiskMap* GetQuestionMapEntry(vector <qtm_data_file_ns::QtmDataDiskMap*> & qtm_datafile_question_disk_map,
		string name);
vector<qtm_data_file_ns::QtmDataDiskMap*> GetQuestionMapEntryArrayQ(vector <qtm_data_file_ns::QtmDataDiskMap*> & qtm_datafile_question_disk_map,
		string name);
...
//CompoundStatement::GenerateCode()
// Generating array declarations: flagGeneratedQuestionDefinitions_: 0, qscript_parser::for_loop_max_counter_stack.size(): 0, counterContainsQuestions_: 4, flagIsAForBody_: 0, source lineNo_: 31
NamedStubQuestion * q2;
NamedStubQuestion * q3;
NamedStubQuestion * q4;
//CompoundStatement::GenerateCode()
// Generating array declarations: flagGeneratedQuestionDefinitions_: 0, qscript_parser::for_loop_max_counter_stack.size(): 0, counterContainsQuestions_: 1, flagIsAForBody_: 1, source lineNo_: 51
//CompoundStatement::GenerateCode()
// Generating array declarations: flagGeneratedQuestionDefinitions_: 0, qscript_parser::for_loop_max_counter_stack.size(): 0, counterContainsQuestions_: 1, flagIsAForBody_: 1, source lineNo_: 54
vector<int32_t> list_q6_array_bounds;
ArrayQuestion q6_list;
DummyArrayQuestion* dum_q6;
XtccSet xs_0;
//CompoundStatement::GenerateCode()
// Generating array declarations: flagGeneratedQuestionDefinitions_: 0, qscript_parser::for_loop_max_counter_stack.size(): 0, counterContainsQuestions_: 0, flagIsAForBody_: 1, source lineNo_: 60
//CompoundStatement::GenerateCode()
// Generating array declarations: flagGeneratedQuestionDefinitions_: 0, qscript_parser::for_loop_max_counter_stack.size(): 0, counterContainsQuestions_: 0, flagIsAForBody_: 0, source lineNo_: 64
//CompoundStatement::GenerateCode()
// Generating array declarations: flagGeneratedQuestionDefinitions_: 0, qscript_parser::for_loop_max_counter_stack.size(): 0, counterContainsQuestions_: 0, flagIsAForBody_: 0, source lineNo_: 70
//CompoundStatement::GenerateCode()
// Generating array declarations: flagGeneratedQuestionDefinitions_: 0, qscript_parser::for_loop_max_counter_stack.size(): 0, counterContainsQuestions_: 1, flagIsAForBody_: 1, source lineNo_: 75
vector<int32_t> list_q5_array_bounds;
ArrayQuestion q5_list;
DummyArrayQuestion* dum_q5;

TheQuestionnaire() 
: list_q6_array_bounds(1),list_q5_array_bounds(1)
{
...
q1 = new NamedStubQuestion(QUESTION_TYPE, 8,string( "q1"), text_expr_vec,spn,1,INT32_TYPE,&yn, QuestionAttributes(false , false), true);
}
qscript_temp_20.push_back( stub_pair("5 agree strongly", 5));
qscript_temp_20.push_back( stub_pair("4 agree somewhat", 4));
qscript_temp_20.push_back( stub_pair("3 Neutral", 3));
qscript_temp_20.push_back( stub_pair("2 Disagree ", 2));
qscript_temp_20.push_back( stub_pair("1 Disagree strongly", 1));
...
q2 = new NamedStubQuestion(QUESTION_TYPE, 36,string( "q2"), text_expr_vec,mpn,5,INT32_TYPE,&veg, QuestionAttributes(false , false), true);
}
{
vector<TextExpression *> text_expr_vec;
text_expr_vec.push_back(new TextExpression(string("Which veg dou eat?")));
q3 = new NamedStubQuestion(QUESTION_TYPE, 41,string( "q3"), text_expr_vec,mpn,5,INT32_TYPE,&veg, QuestionAttributes(false , false), false);
}
{
vector<TextExpression *> text_expr_vec;
text_expr_vec.push_back(new TextExpression(string("Which is ur fav veg?")));
text_expr_vec.push_back( new TextExpression(q3) ); /*  -NxD- */
q4 = new NamedStubQuestion(QUESTION_TYPE, 47,string( "q4"), text_expr_vec,spn,1,INT32_TYPE,&veg, QuestionAttributes(false , false), false);
}
...
RangeQuestion * q6 = new RangeQuestion(QUESTION_ARR_TYPE, 57,string( "q6"), text_expr_vec,spn,1,INT32_TYPE,xs_0, stack_of_loop_indices , dum_q6, QuestionAttributes(false , false), true);
...
NamedStubQuestion * q5 = new NamedStubQuestion(QUESTION_ARR_TYPE, 77,string( "q5"), text_expr_vec,spn,1,INT32_TYPE,&agree_5, stack_of_loop_indices , dum_q5, QuestionAttributes(false , false), true);
...
/* -- */BaseText btxt("q3 oper_in unhandled case leftOperand2_ is oper_name 1608, src/expr.cpp, virtual void Binary2Expression::PrintExpressionText(std::ostringstream&)", true,q3 );
...
base_text_vec.push_back(BaseText("Not q3 oper_in unhandled case leftOperand2_ is oper_name 1608, src/expr.cpp, virtual void Binary2Expression::PrintExpressionText(std::ostringstream&)"));
{
}
base_text_vec.pop_back();
}
...
variable_file	<< "int serial 1" << endl	<< "int driver_code 1" << endl	<< "int proc 1" << endl	<< endl;
}{// PrintBrandRankEdit
{
...
top_level_write_qax(jno, qtm_datafile_question_disk_map);
...
{
/* compound statement on line no: 2 questionsInBlock_, size:6 q1, q2, q3, q4, q6, q5,  */
int32_t i1;
...
/* virtual void named_attribute_list::GenerateCode(StatementCompiledCode&), src/named_attributes.cpp, 27*/
/* if_nest_level: 1 */
/*if (!q1->isAnswered_) {
cerr << "runtime error using unanswered question in expression: " << "q1" << endl;
}*/
int32_t qscript_temp_22 = q1->isAnswered_ ? *q1->input_data.begin() : INT_MAX;
if (qscript_temp_22==1) {// ifStatementStack.size(): 0
// question_list_else_body :
{
/* compound statement on line no: 31 questionsInBlock_, size:4 q2, q3, q4, q6,  */
...
int32_t consolidated_for_loop_index_2 = i1;
/* compound statement on line no: 51 questionsInBlock_, size:1 q6,  */
int32_t code = i1+1;
/* if_nest_level: 2 */
if (/* virtual void Binary2Expression::PrintExpressionCode(ExpressionCompiledCode&), 1730, src/expr.cpp    */
q3->input_data.find(code) != q3->input_data.end()) {// ifStatementStack.size(): 0
// question_list_else_body :
{
/* compound statement on line no: 54 questionsInBlock_, size:1 q6,  */
...
int32_t qscript_temp_28 = q6_list.questionList[i1]->isAnswered_ ? *q6_list.questionList[i1]->input_data.begin() : INT_MAX;
sum = sum+qscript_temp_28;
...
// IfStatement nestLevel_: 4
// IfStatement forNestLevel_: 1
// q6: , nestLevel_: // q6: , forNestLevel_: 1
// enclosingCompoundStatement_ is CompoundStatement 
// and enclosingCompoundStatement_ is part of a for loop 
q6_list.questionList[i1]->isAnswered_ = false;
// **************** 
{
/* compound statement on line no: 60 questionsInBlock_, size:0  */
/* ENTER ExpressionStatement::GenerateCode */
1;
}
}
...
// question_list_else_body :
{
/* compound statement on line no: 64 questionsInBlock_, size:0  */
...
/* generate for loop code here */
// enclosingCompoundStatement_ is IfStatement 
for (int32_t i1= 0;i1 < 5; ++ i1) {
q6_list.questionList[i1]->isAnswered_ = false;
}
// **************** 
{
/* compound statement on line no: 70 questionsInBlock_, size:0  */
/* ENTER ExpressionStatement::GenerateCode */
1;
}
}
...
int32_t consolidated_for_loop_index_7 = i1;
/* compound statement on line no: 75 questionsInBlock_, size:1 q5,  */
/* ENTER: AbstractQuestion::PrintSetupBackJump() : for_bounds_stack.size():1 */
lab_q5:
...
if(user_navigation == NAVIGATE_PREVIOUS){
		AbstractQuestion * target_question = ComputePreviousQuestion(q5_list.questionList[consolidated_for_loop_index_7]);
		if(target_question == 0)
		goto label_eval_q5;
		else {
		jumpToQuestion = target_question->questionName_;
	...
	cout << "saved partial data\n";
		...
		if (q5_list.questionList[consolidated_for_loop_index_7]->isAnswered_==false) {
		cout << "question has not been answered ... jumping back\n";
			goto label_eval_q5;
		}
}
 else { 
last_question_answered = q5_list.questionList[consolidated_for_loop_index_7];
}
}
}
}
...
{
	for (int i=0; i<question_list.size(); ++i) {
		if (question_list[i]->questionName_ == qname) {
			DummyArrayQuestion * dq1 = dynamic_cast<DummyArrayQuestion*>(question_list[i]);
			if (dq1) {
				return dq1;
			}
		}
